...
RSpec.describe MetadataField, type: :model do
  ...
  describe 'validations' do
    ...
    it { should validate_presence_of(:metadata_template) }
    
    it 'validates field_type inclusion' do
      should validate_inclusion_of(:field_type).in_array(%w[text number date boolean select file])
    end
  ...
  describe 'instance methods' do
    ...
    describe '#select_options_array' do
      ...
      it 'parses select options from string' do
        field.update(field_type: 'select', select_options: 'Option 1,Option 2,Option 3')
        expect(field.select_options_array).to eq(['Option 1', 'Option 2', 'Option 3'])
      end

      it 'handles nil select_options' do
        field.update(field_type: 'select', select_options: nil)
        expect(field.select_options_array).to eq([])
      end
    end
    ...
    describe '#html_input_type' do
      it 'returns correct input type for each field type' do
        ...
        expect(create(:metadata_field, field_type: 'number').html_input_type).to eq('number')
  ...
  describe 'callbacks' do
    it 'normalizes select options before save' do
      ...
      expect(field.select_options).to eq('Option 1,Option 2,Option 3')
    end
  end
end