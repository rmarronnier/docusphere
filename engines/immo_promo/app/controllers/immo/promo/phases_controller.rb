class Immo::Promo::PhasesController < Immo::Promo::ApplicationController
  ...
  def create
    ...
    if @phase.save
      redirect_to immo_promo_project_phase_path(@project, @phase), notice: 'Phase créée avec succès.'
    else
  ...
  def update
    ...
    if @phase.update(phase_params)
      redirect_to immo_promo_project_phase_path(@project, @phase), notice: 'Phase mise à jour avec succès.'
    else
  ...
  def destroy
    ...
    if @phase.destroy
      redirect_to immo_promo_project_path(@project), notice: 'Phase supprimée avec succès.'
    else
      redirect_to immo_promo_project_phase_path(@project, @phase), alert: 'Impossible de supprimer la phase.'
    end
  end
  ...
  def complete
    ...
    if @phase.can_start? && @phase.update(status: 'completed', actual_end_date: Time.current)
      redirect_to immo_promo_project_phase_path(@project, @phase), notice: 'Phase marquée comme terminée.'
    else
      redirect_to immo_promo_project_phase_path(@project, @phase), alert: 'Impossible de terminer la phase.'
    end
  end
