class Immo::Promo::TasksController < Immo::Promo::ApplicationController
  ...
  def create
    ...
    if @task.save
      redirect_to immo_promo_project_phase_task_path(@project, @phase, @task), notice: 'Tâche créée avec succès.'
    else
  ...
  def update
    ...
    if @task.update(task_params)
      redirect_to immo_promo_project_phase_task_path(@project, @phase, @task), notice: 'Tâche mise à jour avec succès.'
    else
  ...
  def destroy
    ...
    if @task.destroy
      redirect_to immo_promo_project_phase_path(@project, @phase), notice: 'Tâche supprimée avec succès.'
    else
      redirect_to immo_promo_project_phase_task_path(@project, @phase, @task), alert: 'Impossible de supprimer la tâche.'
    end
  end
  ...
  def complete
    ...
    if @task.update(status: 'completed', actual_end_date: Time.current)
      redirect_to immo_promo_project_phase_task_path(@project, @phase, @task), notice: 'Tâche marquée comme terminée.'
    else
      redirect_to immo_promo_project_phase_task_path(@project, @phase, @task), alert: 'Impossible de terminer la tâche.'
    end
  end
  ...
  def assign
    ...
    if @task.update(assigned_to_id: params[:user_id])
      redirect_to immo_promo_project_phase_task_path(@project, @phase, @task), notice: 'Tâche assignée avec succès.'
    else
      redirect_to immo_promo_project_phase_task_path(@project, @phase, @task), alert: 'Impossible d\'assigner la tâche.'
    end
  end
