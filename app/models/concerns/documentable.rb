module Documentable
  ...
  included do
    # Polymorphic association with Document model
  ...
  def attach_document(file, category: 'project', user:, title: nil, description: nil)
    ...
    # Trigger document processing if service exists
    if defined?(DocumentIntegrationService)
      DocumentIntegrationService.new(document, self).process_document
    elsif defined?(DocumentProcessingService)
  ...
  # Document sharing
  def share_documents_with_user(user, document_ids, permission_level: 'read', shared_by:)
    shares = []
  ...
  def share_document_category_with_users(category, users, permission_level: 'read', shared_by:)
    document_ids = documents_by_category(category).pluck(:id)
    shares = []
    users.each do |user|
      shares.concat(share_documents_with_user(user, document_ids, permission_level: permission_level, shared_by: shared_by))
    end
    shares
  end
  ...
  # Check if user can read documents attached to this entity
  def can_read_documents?(user)
    # Delegate to the entity's policy
    Pundit.policy(user, self).show?
  end

  # Check if user can manage documents attached to this entity
  def can_manage_documents?(user)
    # Delegate to the entity's policy
    Pundit.policy(user, self).update?
  end
  ...
  def document_statistics
    {
      ...
      total_size: documents.joins(file_attachment: :blob).sum('active_storage_blobs.byte_size')
    }
  end
  ...
  def default_document_space
    # Find or create a space for this entity
    ...
    Space.find_or_create_by(
      ...
      space.slug = space_name.parameterize
    end
  end
